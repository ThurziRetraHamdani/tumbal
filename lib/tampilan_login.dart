...
import 'package:tumbal/tampilan_register.dart';
import 'package:shared_preferences/shared_preferences.dart';

class Login extends StatefulWidget {
  static String nama = "";
  final SharedPreferences pref;
  const Login(this.pref, {Key? key}) : super(key: key);
    ...
    for (int i = 0; i < DummyData.data.length; i++) {
      if (usernameInput == DummyData.data[i]['username']) {
        if (passwordInput == DummyData.data[i]['password']) {
          ...
          Login.nama = DummyData.data[i]['nama'];
        ...
        child: Column(
          children: [
            ...
            InkWell(onTap:(() {
               Navigator.pushReplacement(
                    context,
                    MaterialPageRoute(builder: (_) => register(widget.pref)),
                  );
